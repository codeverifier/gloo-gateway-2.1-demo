...
wait_for_successful_resp() {
    local address=$1
    until curl -s -f -o /dev/null $address
    do
        sleep 5
    done
}
...
install_external_dns() {
    ...
    validate_env_var EXTERNAL_DNS_HELM_VERSION "External DNS version is not set with \$EXTERNAL_DNS_HELM_VERSION"
...
install_cert_manager() {
    ...
    validate_env_var CERT_MANAGER_HELM_VERSION "Cert manager version is not set with \$CERT_MANAGER_HELM_VERSION"
    ...
    kubectl --context ${context} \
        apply -f https://github.com/cert-manager/cert-manager/releases/download/$CERT_MANAGER_HELM_VERSION/cert-manager.crds.yaml
...
install_vault() {
    ...
    kubectl --context ${context} \
        wait pod vault-0 -n vault --for=condition=ready
...
install_keycloak() {
    ...
    local context=$1
    validate_env_var context "Kubernetes context is not set"
    validate_env_var KEYCLOAK_HELM_VERSION "Keycloak Helm version is not set"

    # Deploy Vault
    helm repo add bitnami-repo https://charts.bitnami.com/bitnami
    helm repo update
    envsubst < <(cat $DIR/keycloak/keycloak-helm-values.yaml) | helm install keycloak bitnami-repo/keycloak -n keycloak \
        --kube-context ${context} \
    ...
    kubectl --context ${context} \
        wait pod keycloak-0 -n keycloak --for=condition=ready

    wait_for_lb_address $context "keycloak" "keycloak"

    sleep 120

    export ENDPOINT_KEYCLOAK="keycloak.${DOMAIN_NAME}:80"
    export KEYCLOAK_URL="http://${ENDPOINT_KEYCLOAK}"
    echo export KEYCLOAK_URL=$KEYCLOAK_URL > $DIR/../_output/keycloak_env.sh

    wait_for_successful_resp $KEYCLOAK_URL
